...
public class RegisterActivity extends AppCompatActivity {
    ...
    private void setupTabLayout() {
        ...
        // Настраиваем слушатель вкладок
        tabLayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            @Override
            public void onTabSelected(TabLayout.Tab tab) {
                if (tab.getPosition() == 0) {
                    // Нажата вкладка "Login"
                    startActivity(new Intent(RegisterActivity.this, LoginActivity.class));
                    finish();
                } else {
                    // Нажата вкладка "Sign Up" (текущая)
                    // Мы уже находимся на этой вкладке
                }
            }
        ...
        // Устанавливаем вторую вкладку (Sign Up) активной
            ...
            public void onClick(View v) {
                ...
                FirebaseAuth.getInstance().createUserWithEmailAndPassword(email, password)
                        .addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                            ...
                            public void onComplete(@NonNull Task<AuthResult> task) {
                                ...
                                if (task.isSuccessful()) {
                                    ...
                                    // Используем общий DatabaseReference из FirebaseDataManager
                                    DatabaseReference userRef = com.example.emo.firebase.FirebaseDataManager.getDatabaseReference()
                                            .child("Users")
                                            .child(userId);
                                            ...
                                            .addOnSuccessListener(aVoid -> {
                                                ...
                                                userRef.get().addOnSuccessListener(dataSnapshot -> {
                                                    if (dataSnapshot.exists() &&
                                                            ...
                                                            dataSnapshot.hasChild("termsAccepted")) {
                                                        Log.d(TAG, "Данные успешно проверены в базе: " + dataSnapshot.getValue());
                                                        binding.progressBar.setVisibility(View.GONE);
                                                        binding.signUpBtn.setEnabled(true);
                                                        isRegistering = false;
                                                        ...
                                                        Log.e(TAG, "setValue: Проверка данных не удалась или данные отсутствуют.");
                                                        binding.progressBar.setVisibility(View.GONE);
                                                        binding.signUpBtn.setEnabled(true);
                                                        isRegistering = false;
                                                        Toast.makeText(getApplicationContext(), "Регистрация прошла, но не удалось проверить данные. Попробуйте войти.", Toast.LENGTH_LONG).show();
                                                    }
                                                }).addOnFailureListener(e -> {
                                                    handler.removeCallbacks(timeoutRunnable);
                                                    Log.e(TAG, "setValue failed during verification: " + e.getMessage());
                                                    binding.progressBar.setVisibility(View.GONE);
                                                    binding.signUpBtn.setEnabled(true);
                                                    isRegistering = false;
                                                    Toast.makeText(getApplicationContext(), "Ошибка при проверке данных: " + e.getMessage(), Toast.LENGTH_LONG).show();
                                                });
                                            })
                                            .addOnFailureListener(e -> {
                                                handler.removeCallbacks(timeoutRunnable);
                                    ...
                                    if (exception instanceof FirebaseAuthUserCollisionException) {
                                        errorMessage = "Этот email уже зарегистрирован. Пожалуйста, войдите или используйте другой email.";
                                    } else if (exception instanceof FirebaseAuthInvalidCredentialsException) {
                                        errorMessage = "Неверный формат email или слабый пароль. Пароль должен быть не менее 6 символов.";
                                    } else {
                                        errorMessage = "Ошибка регистрации: " + exception.getMessage();
                                    }
                                    Log.e(TAG, "Регистрация не удалась: " + errorMessage);
                        ...
                        .addOnFailureListener(e -> {
                            ...
                            Log.e(TAG, "createUserWithEmailAndPassword failed: " + errorMessage);
                            Toast.makeText(getApplicationContext(), errorMessage, Toast.LENGTH_LONG).show();
                        });
            }
        });
    ...
    private void setupPasswordToggle() {
        ...
        binding.passwordEt.setOnTouchListener((v, event) -> {
            ...
            if (event.getAction() == android.view.MotionEvent.ACTION_UP) {
                if (event.getRawX() >= (binding.passwordEt.getRight() - binding.passwordEt.getCompoundDrawables()[DRAWABLE_RIGHT].getBounds().width() - binding.passwordEt.getPaddingRight())) {
                    // Переключение видимости пароля
                    if (binding.passwordEt.getInputType() == (android.text.InputType.TYPE_CLASS_TEXT | android.text.InputType.TYPE_TEXT_VARIATION_PASSWORD)) {
                        // Показать пароль
                        binding.passwordEt.setInputType(android.text.InputType.TYPE_CLASS_TEXT | android.text.InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD);
                    } else {
                        // Скрыть пароль
                        binding.passwordEt.setInputType(android.text.InputType.TYPE_CLASS_TEXT | android.text.InputType.TYPE_TEXT_VARIATION_PASSWORD);
                    }
        ...
        binding.confirmPasswordEt.setOnTouchListener((v, event) -> {
            ...
            if (event.getAction() == android.view.MotionEvent.ACTION_UP) {
                if (event.getRawX() >= (binding.confirmPasswordEt.getRight() - binding.confirmPasswordEt.getCompoundDrawables()[DRAWABLE_RIGHT].getBounds().width() - binding.confirmPasswordEt.getPaddingRight())) {
                    // Переключение видимости пароля
                    if (binding.confirmPasswordEt.getInputType() == (android.text.InputType.TYPE_CLASS_TEXT | android.text.InputType.TYPE_TEXT_VARIATION_PASSWORD)) {
                        // Показать пароль
                        binding.confirmPasswordEt.setInputType(android.text.InputType.TYPE_CLASS_TEXT | android.text.InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD);
                    } else {
                        // Скрыть пароль
                        binding.confirmPasswordEt.setInputType(android.text.InputType.TYPE_CLASS_TEXT | android.text.InputType.TYPE_TEXT_VARIATION_PASSWORD);
                    }
                    // Сохраняем позицию курсора
